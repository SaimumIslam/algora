...
@Composable
fun DataStructuresScreen(onTopicClick: (String) -> Unit) {
    ...
    val sections = remember {
        DataStructuresCategories.all.map { category ->
            BrowserSection(category, DataStructuresTopics.topics.filter { it.categoryId == category.id })
        }
    }

    CategoryBrowserScreen(
        screenTitle = "Data Structures",
        sections = sections,
